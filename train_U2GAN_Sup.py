...
import time
import datetime
from model_U2GAN_Sup_k_hops_multiGNNlayers import U2GAN
...
def preprocess_graphpool(batch_graph):
    ...
    graph_pool = coo_matrix((elem, (idx_row, idx_cl)), shape=(len(batch_graph), start_idx[-1])).toarray()
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        u_gan = U2GAN(num_hidden_layers=args.num_hidden_layers,
        ...
        write_acc.write('\nepoch ' + str(idx_epoch) + ' fold ' + str(args.fold_idx) + ' Max_acc ' + str(max_acc) + '%\n')
        write_acc.close()
